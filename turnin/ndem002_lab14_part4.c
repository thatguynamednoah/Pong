 ...
 *
 *	Demo Link: https://youtu.be/P8-Fd1GRDpA
 */
#include <avr/io.h>
#include "timer.h"
#include "scheduler.h"
				...
				if (gamePaused == 0) {
					if (aiEnable == 1) {
						...
						if ((xBallCoord == 0x20) && (hasLeft == 0)) {
							...
							if (yTrack == 2) {
								if ((determineMovement < 6) || (determineMovement > 7)) {
									if (yLeftPaddleCoord == 0xF8) {
										yLeftPaddleCoord = ShiftDown(yLeftPaddleCoord);
									}
									else if (yLeftPaddleCoord == 0xF1) {
										if (determineMovement < 3) {
											yLeftPaddleCoord = ShiftDown(yLeftPaddleCoord);
										}
										else {
